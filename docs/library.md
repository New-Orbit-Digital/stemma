...
6. **Reuse the catalog's breeder spelling** (L1-C 2026-09-25, batches 1–3: 3 cards). Before setting `breeder`, search existing `breeder` values on `main` and reuse an exact match. Use the breeding company's name, not a retail brand and not a strain name: "Cookie Fam Genetics" (not "Cookies"), "Grand Daddy Purp" (not "Grand Daddy Purple"). A brand can still be named in prose, linked as `[[breeder:Cookie Fam Genetics|Cookies]]`. (Seen: london-pound-cake, biscotti, candyland. Batch 4 also wrote "T.H. Seeds" where batch 8 had "T.H.Seeds". Both slug to `t-h-seeds`, so `build.py` aborted.) Near-spellings with *different* slugs pass `build.py` silently, so compare after dropping a leading "The", punctuation, spaces, and possessives. (Seen: "TH Seeds" / "T.H.Seeds", "Cali Connection" / "The Cali Connection", "Soma Seeds" / "Soma's Sacred Seeds". All consolidated in #70.)
...
9. **Don't restate `traditional_label` in the summary** (L1-C 2026-09-25, batch 5: 20 of 25 cards). "It is grown as a mostly indica hybrid" repeats a field the page already shows and spends the 400 characters. End on a fact about identity, origin, lineage, or history; if there is none, a two-sentence summary is fine. (Seen across batch 5, for example zookies, birthday-cake, cookies-kush, obama-runtz. Merged cards aren't reworked for this alone.)
